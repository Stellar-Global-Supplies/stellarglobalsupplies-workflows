...
function resolveSecret(val) {
  ...
  async function bedrockInvoke(env, prompt, system, maxTokens) {
    const region    = resolveSecret(env.BEDROCK_REGION) || 'us-east-1'
    ...
    const headers = await signRequest({
      ...
      service:    'bedrock',
      region,
      accessKeyId:     resolveSecret(env.BEDROCK_ACCESS_KEY_ID),
      secretAccessKey: resolveSecret(env.BEDROCK_SECRET_ACCESS_KEY),
    })
